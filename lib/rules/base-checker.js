    ...
    if (userConfig) {
      if (Array.isArray(userConfig)) {
        ...
        if (!validate(userConfig)) {
          ...
          console.warn(`[solhint] Warning: invalid configuration for rule '${ruleId}':\n${messages}`)
        }
      } else if (!['off', 'warn', 'error'].includes(userConfig)) {
        console.warn(`[solhint] Warning: rule '${ruleId}' level is '${userConfig}'; should be one of "error", "warn" or "off"`)
      }
    }
  }
