...
const notifyRuleDoesntExist = _.memoize(ruleId => {
  console.warn(`[solhint] Warning: Rule '${ruleId}' doesn't exist`)
})
...
function loadPlugin(pluginName, { reporter, config, inputSrc, fileName }) {
  ...
  try {
    ...
    console.error(`[solhint] Error: Could not load solhint-plugin-${pluginName}, make sure it's installed.`)
    process.exit(1)
  }

  if (!Array.isArray(plugins)) {
    console.warn(`[solhint] Warning: Plugin solhint-plugin-${pluginName} doesn't export an array of rules. Ignoring it.`)
    return []
  }
...
function ruleEnabled(coreRule, rules) {
  ...
  // console.log(`ruleId ${coreRule.ruleId} rulevalue ${ruleValue}`)
  // console.log(`'${coreRule.ruleId}':'off',`)
